...
public class Heap<T> {
    ...
    public Heap(ArrayList<T> data, Comparator<T> comparator){
    	// have to do something with array[0] 
    ...
    public void insert(T key){
        heapSize++;
        heap.ensureCapacity(heapSize); // increase size of array capacity by one
        heapIncreaseKey(heapSize, key); // should be passing in last, index = heapSize
    } 
    ...
    private void heapIncreaseKey(int index, T key){
        ...
        while(index > 1 && 	comparator.compare(heap.get(parent), heap.get(index)) < 0){
        	T temp = heap.get(index);
